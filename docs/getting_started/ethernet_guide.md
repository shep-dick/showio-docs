...
Elsewhere in the docs, you will typically see examples written using addresses in the 192.168.0.0/24 range. 

Windows, Mac OS, Linux machines, and mobile devices all have their own ways to set static IP addresses. Each should have their own guide. Information for configuring a ShowIO node's IP address can be found here *add link to... manual? getting started?*.
...
Most computers will be set to use DHCP-assigned addresses by default, requesting and receiving an address from a network switch with a built in DHCP server. Instructions to configure a ShowIO node to use DHCP are here *add link to... manual? getting started?*.
